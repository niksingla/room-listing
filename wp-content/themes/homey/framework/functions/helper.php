...
if( !function_exists( 'homey_pagination_halfmap' ) ){
    function homey_pagination_halfmap($pages = '', $paged = '', $range = 2 ) {
        ...
        if( 1 != $pages ){
            ...
            echo ''.( $paged > 1 ) ? '<li><a data-homeypagi="'.$prev.'" rel="Prev" href="#"><span aria-hidden="true"><i class="homey-icon homey-icon-arrow-left-1"></i></span></a></li>' : '<li class="disabled"><a aria-label="Previous"><span aria-hidden="true"><i class="homey-icon homey-icon-arrow-left-1"></i></span></a></li>';

            for ( $i = 1; $i <= $pages; $i++ ) {
                if ( 1 != $pages &&( !( $i >= $paged+$range+1 || $i <= $paged-$range-1 ) || $pages <= $showitems ) )
                {
                    if ( $paged == $i ){
                        echo '<li class="active"><a data-homeypagi="'.$i.'" href="#">'.$i.' </a></li>';
                    } else {
                        echo '<li><a data-homeypagi="'.$i.'" href="#">'.$i.'</a></li>';
                    }
                }
            }
            echo ''.( $paged < $pages ) ? '<li><a data-homeypagi="'.$next.'" rel="Next" href="'.get_pagenum_link($next).'"><span aria-hidden="true"><i class="homey-icon homey-icon-arrow-right-1"></i></span></a></li>' : '';

            echo ''.( $paged < $pages-1 &&  $paged+$range-1 < $pages && $showitems < $pages ) ? '<li><a data-homeypagi="'.$pages.'" rel="Last" href="#"><span aria-hidden="true"><i class="homey-icon homey-icon-arrow-right-1"></i></span></a></li>' : '';
            echo '</ul>';
            echo '</nav>';
