...
if (!function_exists('homey_get_listing_data')) {
    function homey_get_listing_data($field, $id = 0)
    {
        $prefix = 'homey_';
        $post_id = get_the_ID();
    ...
    function homey_listing_permalink($listing_id = null)
    {
        ...
        if ($homey_booking_type == 'per_hour') {
            ...
            if (isset($_REQUEST['arrive']) || isset($_REQUEST['start'])) {
                ...
                $adult_guest = isset($_REQUEST['adult_guest']) ? $_REQUEST['adult_guest'] : '';
                $child_guest = isset($_REQUEST['child_guest']) ? $_REQUEST['child_guest'] : '';
                ...
                $link = add_query_arg('adult_guest', (trim($adult_guest)), $link);
                $link = add_query_arg('child_guest', (trim($child_guest)), $link);
            }
            ...
            if (isset($_REQUEST['arrive']) || isset($_REQUEST['depart'])) {
                ...
                if (!empty(trim($adult_guest))) {
                    $link = add_query_arg('adult_guest', (trim($adult_guest)), $link);
                }
                if (!empty(trim($child_guest))) {
                    $link = add_query_arg('child_guest', (trim($child_guest)), $link);
                }
            }
        }
...
if (!function_exists('homey_get_dates_for_booking')) {
    function homey_get_dates_for_booking()
    {
        ...
        $dates_array['adult_guest'] = isset($_GET['adult_guest']) ? $_GET['adult_guest'] : '';
        $dates_array['child_guest'] = isset($_GET['child_guest']) ? $_GET['child_guest'] : '';
        ...
        if ($the_query->have_posts()) :
            while ($the_query->have_posts()) : $the_query->the_post();
                ...
                if ($listing_style == 'card') {
                    ...
                    get_template_part('template-parts/listing/listing-item-v2');
                } else {
...
if (!function_exists('save_listing_as_draft')) {
    function save_listing_as_draft()
    {
        ...
        if ($listing_id > 0) {
            ...
            // Address
            if (isset($_POST['listing_address'])) {
                $full_address_arr['listing_address'] = sanitize_text_field($_POST['listing_address']);
                update_post_meta($listing_id, $prefix . 'listing_address', sanitize_text_field($_POST['listing_address']));
            }
            ...
            //AptSuit
            if (isset($_POST['aptSuit'])) {
                $full_address_arr['aptSuit'] = sanitize_text_field($_POST['aptSuit']);
                update_post_meta($listing_id, $prefix . 'aptSuit', sanitize_text_field($_POST['aptSuit']));
            }
            ...
            if (isset($_POST['zip'])) {
                ...
                $full_address_arr['zip'] = sanitize_text_field($_POST['zip']);
                update_post_meta($listing_id, $prefix . 'zip', sanitize_text_field($_POST['zip']));
            }

            // Country
            if (isset($_POST['country'])) {
                $full_address_arr['country'] = sanitize_text_field($_POST['country']);
                $listing_country = sanitize_text_field($_POST['country']);
                $country_id = wp_set_object_terms($listing_id, $listing_country, 'listing_country');
            }
            ...
            // State
            if (isset($_POST['administrative_area_level_1'])) {
                $full_address_arr['listing_state'] = sanitize_text_field($_POST['administrative_area_level_1']);
            ...
            // City
            if (isset($_POST['locality'])) {
                $full_address_arr['listing_city'] = sanitize_text_field($_POST['listing_city']);
            ...
            // Area
            if (isset($_POST['neighborhood'])) {
                $full_address_arr['listing_area'] = sanitize_text_field($_POST['listing_area']);
    ...
    function homey_get_listing_featured($listing_id)
    {
        ...
        if ($featured == 1) {
            ...
            if (is_singular('listing')) {
                $html_output = '<span class="label label-featured">' . $homey_local['featured_label'] . '</span>';
            } else {
                $html_output = '<span class="label-wrap top-left">
                    <span class="label label-featured">' . $homey_local['featured_label'] . '</span>
                </span>';
            }
        }
        return $html_output;
...
if (!function_exists('homey_listing_sort')) {
    function homey_listing_sort($query_args)
    {
        ...
        if (isset($_GET['sortby'])) {
            ...
            if (is_page_template(array('template/template-listing-list.php', 'template/template-listing-list-v2.php', 'template/template-listing-grid.php', 'template/template-listing-grid-v2.php', 'template/template-listing-card.php', 'template/template-listing-sticky-map.php'))) {
...
if (!function_exists('homey_check_is_favorite')) {
    // a:1:{i:0;i:543;}
    function homey_check_is_favorite($listing_id = 0)
    {
        global $current_user;
        wp_get_current_user();
        if (isset($current_user->ID)) {
            ...
            if ($current_prop_fav && in_array($listing_id, $current_prop_fav)) {
                return 1;
            }
        }
...
if (!function_exists('homey_generate_invoice')):
    function homey_generate_invoice($billingFor, $billionType, $list_pack_resv_ID, $invoiceDate, $userID, $featured, $upgrade, $paypalTaxID, $paymentMethod, $amount_paid = 0)
    {
        ...
        if ($billingFor == 'reservation') {
            ...
            $listing_owner = get_post_meta($list_pack_resv_ID, 'listing_owner', true);
            update_post_meta($inserted_post_id, 'invoice_payment_status', 1);
        }
...
if (!function_exists('homey_copy_images_for_duplicated_post')) {
    function homey_copy_images_for_duplicated_post($listing_id = 0)
    {
        if (!empty($_POST['listing_image_ids']) && is_array($_POST['listing_image_ids'])) {
            ...
            $to_copy = count($_POST['listing_image_ids']); // test
            $copied = 0; // test
            $listing_image_ids = array();
            ...
            foreach ($_POST['listing_image_ids'] as $img_id) {
                ...
                if ($img_id && $file = get_attached_file($img_id)) {
                    ...
                    if (@copy($file, $file2)) {
                        ...
                        // Then add the meta data `homey_listing_images`.
                        add_post_meta($listing_id, 'homey_listing_images', $img_id2);
                        $listing_image_ids[] = intval($img_id);
        ...
        <div class="invoice-detail block">
            <div class="invoice-header clearfix">
                <div class="block-left">
                    <div class="invoice-logo">
                        <?php if (!empty($invoice_logo)) { ?>
                            <img src="<?php echo esc_url($invoice_logo); ?>"
                                 alt="<?php esc_attr_e('logo', 'homey'); ?>">
                        <?php } ?>
                    </div>
                ...
                <div class="block-right">
                    <ul class="list-unstyled">
                        <li>
                            <strong><?php esc_html_e('Invoice:', 'homey'); ?></strong> <?php echo esc_attr($invoice_id); ?>
                        </li>
                        <li>
                            <strong><?php esc_html_e('Date:', 'homey'); ?></strong> <?php echo esc_attr($publish_date); ?>
                        </li>
                        <?php if ($is_reservation_invoice) { ?>
                            <li>
                                <strong><?php esc_html_e('Reservation ID:', 'homey'); ?></strong> <?php echo esc_attr($invoice_item_id); ?>
                            </li>
                        <?php } ?>
                    </ul>
                </div>
        ...
        <div class="print-main-wrap">
            <div class="print-wrap">
                ...
                <div class="print-header">
                    <h1><img src="<?php echo esc_url($print_logo); ?>" width="128" height="30"
                ...
                <div class="top-section">
                    <div class="block">
                            ...
                            <div class="block-right">
                                <span class="item-price">
                                    <?php if ($booking_type == 'per_hour') { ?>
                                        <?php echo homey_formatted_price($hour_price, false, true); ?><sub>
                                        /<?php echo homey_option('glc_hour_label'); ?></sub>
                                    <?php } else { ?>
                                        <?php echo homey_formatted_price($night_price, false, true); ?><sub>
                                        /<?php echo homey_option('glc_day_night_label'); ?></sub>
                                    <?php } ?>
                                </span>
                            </div><!-- block-right -->
                        </div><!-- block-head -->
                        ...
                        <?php if (!empty($full_img)) { ?>
                            <img class="img-responsive" src="<?php echo esc_url($full_img); ?>"
                                 alt="<?php echo esc_attr($title); ?>">

                            <?php if (homey_option('print_qr_code')) { ?>
                                <img class="qr-code img-responsive"
                                     src="https://chart.googleapis.com/chart?chs=105x104&cht=qr&chl=<?php echo esc_url(get_permalink($listing_id)); ?>&choe=UTF-8"
                            ...
                            <div class="block-head">
                                <div class="media">
                                    ...
                                    <div class="media-body">
                                        <h2 class="title"><?php echo homey_option('sn_hosted_by'); ?>
                                            <span><?php echo esc_attr($listing_author['name']); ?></span></h2>

                                        <?php if (!empty($listing_author['city'])) { ?>
                                            <address><i class="homey-icon homey-icon-style-two-pin-marker"
                                                        aria-hidden="true"></i> <?php echo esc_attr($listing_author['city']); ?>
                                            </address>
                                        <?php } ?>
                                                ...
                                                <div class="col-xs-4 col-sm-4 col-md-4 col-lg-4">
                                                    ...
                                                    <dl>
                                                        ...
                                                        if ($current_page_user == 'administrator') { ?>
                                                            ...
                                                            if ($verified) { ?>
                                                                <dd class="text-success"><i
                                                                            class="homey-icon homey-icon-check-circle-1"></i> <?php esc_html_e('Verified', 'homey'); ?>
                                                                </dd>
                                                            <?php } else { ?>
                                                                <dd class="text-danger"><i
                                                                            class="homey-icon homey-icon-close"></i> <?php esc_html_e('Not Verified', 'homey'); ?>
                                                                </dd>
                                                            <?php }
                                                        } ?>
                                                    </dl>
                                    ...
                                    <div class="block-right">
                                        <ul class="detail-list detail-list-2-cols">
                                            <?php if ($hide_labels['sn_id_label'] != 1) { ?>
                                                <li>
                                                    <i class="homey-icon homey-icon-arrow-right-1"
                                                       aria-hidden="true"></i>
                                                    <?php echo esc_attr(homey_option('sn_id_label')); ?>:
                                                    <strong><?php echo esc_attr($listing_id); ?></strong>
                                                </li>
                                            <?php } ?>

                                            <?php if (!empty($guests) && $hide_labels['sn_guests_label'] != 1) { ?>
                                                <li>
                                                    <i class="homey-icon-arrow-right-1" aria-hidden="true"></i>
                                                    <?php echo homey_option('sn_guests_label'); ?>:
                                            ...
                                            <?php if (!empty($bedrooms) && $hide_labels['sn_bedrooms_label'] != 1) { ?>
                                                <li>
                                                    <i class="homey-icon-arrow-right-1" aria-hidden="true"></i>
                                                    <?php echo homey_option('sn_bedrooms_label'); ?>:
                                                    <strong><?php echo esc_attr($bedrooms); ?></strong>
                                                </li>
                                            <?php } ?>

                                            <?php if (!empty($beds) && $hide_labels['sn_beds_label'] != 1) { ?>
                                                <li><i class="homey-icon-arrow-right-1" aria-hidden="true"></i>
                                                    <?php echo homey_option('sn_beds_label'); ?>:
                                            ...
                                            <?php if (!empty($baths) && $hide_labels['sn_bathrooms_label'] != 1) { ?>
                                                <li><i class="homey-icon-arrow-right-1" aria-hidden="true"></i>
                                                    <?php echo homey_option('sn_bathrooms_label'); ?>:
                                            ...
                                            <?php if ($booking_type == 'per_day' || $booking_type == 'per_day_date') { ?>
                                                <?php if (!empty($checkin_after) && $hide_labels['sn_check_in_after'] != 1) { ?>
                                                    <li><i class="homey-icon-arrow-right-1" aria-hidden="true"></i>
                                                        <?php echo homey_option('sn_check_in_after'); ?>:
                                                        <strong><?php echo esc_attr($checkin_after); ?></strong>
                                                    </li>
                                                <?php } ?>

                                                <?php if (!empty($checkout_before) && $hide_labels['sn_check_out_before'] != 1) { ?>
                                                    <li><i class="homey-icon-arrow-right-1" aria-hidden="true"></i>
                                                        <?php echo homey_option('sn_check_out_before'); ?>:
                                                        <strong><?php echo esc_attr($checkout_before); ?></strong>
                                                    </li>
                                                <?php } ?>
                                            <?php } ?>
                                            ...
                                            <?php if ((!empty($room_type) || !empty($listing_type)) && $hide_labels['sn_type_label'] != 1) { ?>
                                                <li><i class="homey-icon-arrow-right-1" aria-hidden="true"></i>
                                                    <?php echo homey_option('sn_type_label'); ?>:
                                                    <strong><?php echo esc_attr($room_type) . ' ' . $slash . ' ' . esc_attr($listing_type); ?></strong>
                                                </li>
                                            <?php } ?>

                                            <?php if (!empty($size) && $hide_labels['sn_size_label'] != 1) { ?>
                                                <li><i class="homey-icon-arrow-right-1" aria-hidden="true"></i>
                                                    <?php echo homey_option('sn_size_label'); ?>:
                                                    <strong><?php echo esc_attr($size) . ' ' . esc_attr($size_unit); ?></strong>
                                                </li>
                                            <?php } ?>
                                        </ul>
                                    </div><!-- block-right -->
                                    ...
                                    <div class="block-right">
                                        <ul class="detail-list detail-list-2-cols">
                                            ...
                                            <?php if (!empty($weekends_price) && $hide_labels['sn_weekends_label'] != 1) { ?>
                                                <li>
                                                    <i class="homey-icon-arrow-right-1" aria-hidden="true"></i>
                                                    <?php echo homey_option('sn_weekends_label'); ?>
                                        ...
                                        <?php foreach ($accomodation as $acc): ?>
                                            <div class="block-col block-col-33 block-accomodation">
                                                ...
                                                <dl>
                                                    <?php
                                                    if ($acc['acc_guests'] > 1) {
                                                        $guests = homey_option('sn_acc_guests_label');
                                                    } else {
                                                        $guests = $homey_local['acc_guest_label'];
                                                    }
                                ...
                                <div class="block-body">
                                    ...
                                    <div class="block-right">
                                        <?php if (!empty($amenities)) { ?>
                                            <p><strong><?php echo esc_attr(homey_option('sn_amenities')); ?></strong>
                                            </p>
                                            <ul class="detail-list detail-list-2-cols">
                                                <?php foreach ($amenities as $amenity): ?>
                                                    <li><i class="homey-icon-arrow-right-1"
                                                           aria-hidden="true"></i> <?php echo esc_attr($amenity->name); ?>
                                                    </li>
                                                <?php endforeach; ?>
                                            </ul>
                                        <?php } ?>
                                        ...
                                        <?php if (!empty($facilities)) { ?>
                                            ...
                                            <ul class="detail-list detail-list-2-cols">
                                                <?php foreach ($facilities as $facility): ?>
                                                    <li><i class="homey-icon-arrow-right-1"
                                                           aria-hidden="true"></i> <?php echo esc_attr($facility->name); ?>
                                                    </li>
                                                <?php endforeach; ?>
                                            </ul>
                                        <?php } ?>
                                    ...
                                    <div class="block-right">
                                        <ul class="rules_list detail-list">
                                            <?php if ($hide_labels['sn_smoking_allowed'] != 1) { ?>
                                                <li>
                                                    <i class="<?php echo esc_attr($smoke_allow); ?>"
                                                       aria-hidden="true"></i>
                                            ...
                                            <?php if ($hide_labels['sn_pets_allowed'] != 1) { ?>
                                                <li>
                                                    <i class="<?php echo esc_attr($pets_allow); ?>"
                                                       aria-hidden="true"></i>
                                            ...
                                            <?php if ($hide_labels['sn_party_allowed'] != 1) { ?>
                                                <li>
                                                    <i class="<?php echo esc_attr($party_allow); ?>"
                                                       aria-hidden="true"></i>
                                            ...
                                            <?php if ($hide_labels['sn_children_allowed'] != 1) { ?>
                                                <li>
                                                    <i class="<?php echo esc_attr($children_allow); ?>"
                                                       aria-hidden="true"></i>
                                        ...
                                        <?php if (!empty($additional_rules)) { ?>
                                            <ul class="detail-list">
                                                <li><strong><?php echo homey_option('sn_add_rules_info'); ?></strong>
                                                </li>
                                        ...
                                        <ul class="detail-list detail-list-2-cols">
                                            <?php if ($booking_type == 'per_hour') { ?>
                                                <li>
                                                    ...
                                                    <?php echo homey_option('sn_min_stay_is'); ?>
                                                    <strong><?php echo esc_attr($min_stay_hours); ?><?php echo esc_attr($min_book_hours_label); ?></strong>
                                                </li>
                                            <?php } else { ?>
                                                <li>
                                                    <i class="homey-icon homey-icon-calendar-3" aria-hidden="true"></i>
                                                    <?php echo homey_option('sn_min_stay_is'); ?>
                                                    <strong><?php echo esc_attr($min_stay_days); ?><?php echo homey_option('sn_night_label'); ?></strong>
                                                </li>
                                                <li>
                                                    <i class="homey-icon homey-icon-calendar-3" aria-hidden="true"></i>
                                                    <?php echo homey_option('sn_max_stay_is'); ?>
                                                    <strong><?php echo esc_attr($max_stay_days); ?><?php echo homey_option('sn_nights_label'); ?></strong>
                                                </li>
                                            <?php } ?>
                                        </ul>
                                    </div><!-- block-right -->
...
add_action('wp_ajax_homey_approve_listing', 'homey_approve_listing');
if (!function_exists('homey_approve_listing')) {
    function homey_approve_listing()
    {
        if (!is_user_logged_in()) {
            exit('ko');
        }
