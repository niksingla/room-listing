...
<div class="user-dashboard-left white-bg"> 
    <div class="navi">
        <ul class="board-panel-menu">
            <?php 
            if(!homey_is_renter())
            $homey_local['m_dashboard_label'] = 'Armaturenbrett';
            if( !empty($dashboard) ) {
            ...
            if( !empty($dashboard_profile) ) {
                if(!homey_is_renter())
                $homey_local['m_profile_label'] = 'Profil';
                echo '<li class="has-child '.esc_attr($ac_profile).'">
                    ...
                    if(!homey_is_renter())
                    $verification_label = 'Überprüfung';
                        if(!homey_is_admin()) {
                            echo '<li><a href="'.esc_url($verification_page).'">'.esc_html__($verification_label, 'homey').'</a></li>';
                        }
                        $password_label = 'Password';
                        if(!homey_is_renter())
                        $password_label = 'Passwort';
                        echo '<li><a href="'.esc_url($password_page).'">'.esc_html__($password_label, 'homey').'</a></li>';
                        $payment_method_label = 'Payment Method';
                        if(!homey_is_renter())
                        $payment_method_label = 'Bezahlverfahren';
                        if($offsite_payment != 0 || $enable_wallet != 0) {
                            echo '<li><a href="'.esc_url($payment_method_setup).'">'.esc_html__($payment_method_label, 'homey').'</a></li>';
                        }
                    echo '</ul>';
            ...
            if(!empty($dashboard_listings)) {
                if(!homey_is_renter())
                $homey_local['m_listings_label'] = 'Meine Einträge';
                ...
                if (!empty($dashboard_reservations)) {
                    if (homey_is_renter()) {
                        ...
                        <a href="' . esc_url($dashboard_reservations) . '">' . esc_html__('Meine Buchungen', 'homey') . '</a>
                    </li>';
                    } else {
                        ...
                        echo '<li class="' . esc_attr($ac_reserv) . '">
                        <a href="' . esc_url($dashboard_reservations) . '">' . esc_html__('Meine Buchungen', 'homey') . ' ' . $new_notification . '</a>
                    </li>';
                    }
                }
                    ...
                    if(homey_is_host()) {
                        if( !empty($dashboard_wallet) ) {
                            ...
                            echo '<li class="'.esc_attr($ac_wallet).' has-child">
                                <a href="'.esc_url($dashboard_wallet).'">'.esc_html__($wallet_label, 'homey').' <i class="homey-icon homey-icon-arrow-down-1"></i></a>
                                <ul>
