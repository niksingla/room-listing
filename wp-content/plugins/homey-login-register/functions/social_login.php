...
if( !function_exists('homey_facebook_login') ):
    ...
    function homey_facebook_login($get_vars){
        ...
        if ( $access_token && !$access_token->isExpired() )
        {
            ...
            $fb_email       = isset($user['email']) ? $user['email'] : '';

            if(empty($fb_email)){
                echo 'Please check the Facebook App settings, or contact to the admin, its look like email is missing.';
                exit;
            }
            ...
            if(isset($user->ID)){
                ...
                if(isset($user->user_login)){
                    ...
                    add_filter( 'authenticate', 'homey_social_nop_auto_login', 3, 10 );
                    homey_social_nop_auto_login($user, $user->user_login, null, $homeURL );
                }
...
function homey_social_nop_auto_login( $user, $username, $password, $url=null ) {
    if ( ! $user ) {
    ...
    if ( $user ) {
        ...
        remove_filter( 'authenticate', 'homey_social_nop_auto_login', 3, 10 );
